...
This section contains the hooks that run at startup and when emacsclient is launched.
...
(use-package org-roam
  ...
  (org-roam-capture-templates '(("f" "Fleeting notes" plain "%?"
                                 ...
                                 "#+STARTUP: content\n* ${title}\nAuthor: %^{Author}\nFull title: %^{FullTitle}\nYear: %^{Year}\nType: %^{Type}\nFull reference: %^{FullReference}\n\n\n** Notes\n\n\n%?"
								   :if-new (file+head "%<%Y%m%d%H%M%S>-${slug}.org" "#+title: ${title}\n"))
                                ("p" "Permanent notes" plain "#+STARTUP: showeverything\n* ${title}\n%?"
                                 :if-new (file+head "%<%Y%m%d%H%M%S>-${slug}.org" "#+title: ${title}\n"))))
